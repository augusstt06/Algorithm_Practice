...
# 핵심 아이디어 : 옆에 있는 값가 비교해서 더 작은 값을 앞으로 보낸다 (값 자리를 바꾼다)
...
# ====> 정렬 하고자 하는 범위를 확인 하면서 그때 그때 정해둔 기준으로 비교해 자리를 바꿔 가면서 정렬 하는 방식을 반복한다.

# 오름 차순 정렬시 : 오름 차순 정렬? 작은 값 부터 차례 대로 => 주어진 범위를 하나 하나 탐색할 때마다 대소를 비교해서 자리를 바꿔가며 진행 => 반복
